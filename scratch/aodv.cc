...
void
AodvExample::CourseChange (std::string context, Ptr<const MobilityModel> mobility)
{
  ...
  //         Simulator::ScheduleNow (&AodvExample::SetupPacketSend, this, nodeid);
  //       }
...
void
AodvExample::SetupPacketReceive (Ipv4Address addr, Ptr <Node> node, uint16_t port)
{
...
void
AodvExample::SetupPacketSend (uint32_t nodeid)
{
...
//  aodv.Install ();
  ...
  Config::Connect ("/NodeList/*/$ns3::MobilityModel/CourseChange", MakeCallback (&AodvExample::CourseChange, this));
  ...
  for (uint32_t i = 0; i < nodes.GetN (); i++)
    {
      ...
      os_pdr << nodeId << "|" << packetsSent[i] << "|" << packetsRcvd[i];
...
AodvExample::InstallApplications ()
{
	...
	//	app.Stop (Seconds (totalTime-0.1));
		...
		uint16_t server1Position = 18; //bottom right
